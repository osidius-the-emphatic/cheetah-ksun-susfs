...
| config/versions.env | URL, гілки та необов’язкові закріплені коміти. |
...
## 3. Дерево вихідного коду Google
...
Рядок версії Google GKI зазвичай містить скорочений коміт Git після `-g`,
наприклад `...-gbd23337e42e7-...`. «Зафіксувати `common/`» означає зробити
перейти саме на цей коміт вихідного коду в локальному Git-репозиторії `common/`; це
не просто вибір гілки `common-android14-6.1`. Спочатку переконайтеся, що коміт
є в локальній історії, потім створіть або перемістіть локальну робочу гілку:
...
Замініть `bd23337e42e7` на скорочений коміт із вашого телефону. Запишіть повний SHA у
`config/versions.env` як `GOOGLE_BASE_COMMIT=...`. Якщо коміт
не знайдений, зупиніться: отримайте правильну історію вихідного коду Google або
визначте відповідну ревізію заводського образу та ядра до будь-якої інтеграції. Наприкінці
перевірте наявність `$KERNEL_CHECKOUT/common`, `$KERNEL_CHECKOUT/build` і
...
після перевірки шляху й фіксації kernel release, що зараз працює на телефоні:
...
`KSUN_BRANCH` для отримання вихідного коду. Необов’язкові
`SUSFS_EXPECTED_COMMIT` і `KSUN_EXPECTED_COMMIT` перетворюють перевірену
збірку на відтворювану. Далі скрипт копіює файли SuSFS, застосовує patch,
створює символічне посилання KSU, прибирає блокування protected-export і запускає Kleaf. Не
застосовуйте додатково застарілий `60_scope-minimized_manual_hooks.patch` чи
...
Для першої збірки не змінюйте URL репозиторіїв, гілки та два необов’язкові поля
очікуваних комітів у `config/versions.env`; ці поля мають залишатися порожніми
до перевірки успішної збірки. Коміт телефона береться з `adb shell uname -r` і
записується у `config/versions.env` як `GOOGLE_BASE_COMMIT` до збирання;
окремої змінної `KERNEL_COMMIT` немає.
...
Файл доказів містить коміти common, SuSFS і KernelSU та SHA-256. Не пакуйте неповний каталог результатів.
...
Якщо раніше використовувався LKM-root, спочатку поверніть заводський `init_boot.img`.
...
Скрипти ці команди не виконують. Гілка `dev-susfs` експериментальна; успішний
тимчасове завантаження не є гарантією стабільності для щоденного використання.
...
експериментальна; успішне тимчасове завантаження не є гарантією стабільності для щоденного використання.
...
програм в upstream-проєкті. Не встановлюйте їх одночасно. Бажано використовувати
...
Порожні поля очікуваних комітів заповнюйте точним SHA лише після першої перевіреної збірки.
...
| Патч SuSFS не застосовується | Ревізія `common/` і `SUSFS_BRANCH` не відповідають патчу для цієї версії. | Зупиніться. Звірте ревізію вихідного коду і зовнішню гілку, прочитайте patch/diff. Не використовуйте fuzz чи часткове накладання. |
...
Це порядок дій для Pixel 7 Pro/GKI 6.1. Через розділену структуру образів
пакується лише `boot.img`, а `init_boot.img` залишається заводським образом для відновлення.
