...
У відповідному `boot.img` Pixel 7 Pro, kernel payload якого перевірено 2026-09-15,
записано release `6.1.157-android14-11-gbd23337e42e7-ab14791245`. Його суфікс
`-g` розгортається у `bd23337e42e794964a89f47596daf1209a25ee1a` — це
закріплена Google base цього репозиторію. «Зафіксувати `common/`» означає
...
`SUSFS_EXPECTED_COMMIT` і `KSUN_EXPECTED_COMMIT` зупиняють процес, якщо tip
гілки більше не відповідає перевіреному знімку. `GOOGLE_BASE_COMMIT` уже
зіставлено з указаним вище kernel payload boot image. Для іншого образу пристрою
перевірте його суфікс із `adb shell uname -r` перед збиранням. Якщо він інший,
зупиніться; не підставляйте інший коміт Google, зберігаючи поточні pins
інтеграції. Окремої змінної `KERNEL_COMMIT` немає.

Далі скрипт копіює файли SuSFS, застосовує patch, створює символічне посилання
...
дозволяють непомітно застосувати patch до іншої upstream-ревізії. Google base
зіставлено з документованим kernel payload boot image, але це саме по собі не доводить,
що поточні pins SuSFS/KernelSU-Next збираються, зберігають ABI або завантажуються.
Оновлюйте pins інтеграції лише після рецензії вихідного коду та чистої збірки з
її файлом доказів.
